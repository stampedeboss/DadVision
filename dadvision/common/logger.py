...
def start(filename=None, level=logging.INFO, timed=False, errorlog=False):
    ...
    if errorlog:
        _error_log = os.path.join(os.path.splitext(os.path.basename(filename))[0], '.error.log')
        error_handler = logging.handlers.RotatingFileHandler(_error_log,
                                                             maxBytes=1000 * 1024,
                                                             backupCount=9)
