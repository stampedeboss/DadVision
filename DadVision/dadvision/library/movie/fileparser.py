...
class FileParser(Library):
    ...
    def getFileDetails(self, fq_name):
        ...
        if 'Trailer' in _parsed_keys and _parse_details.group('Trailer') is not None:
    ...
    def _get_movie_name(self, _parsed_keys, _parse_details):
        ...
        if 'MovieName' in _parsed_keys and _parse_details.group('MovieName') is not None:
...
#        _movie_name = _movie_name.replace("]", "")
        ...
        log.trace('Exiting _get_movie_name {}: Movie Name: {}'.format(self.LogHeader, _movie_name.strip()))
