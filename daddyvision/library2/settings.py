...
class Settings(object):
    ...
    def __init__(self, subject=['common'], update=False):
        ...
        if not os.path.exists(self.NewSeriesDir):
            try:
                os.makedirs(os.path.expanduser(self.NewSeriesDir))
            except OSError, exc:
                log.error("Unable to Create Directory: %s, %s: " % (self.NewSeriesDir, exc))
        ...
        if not os.path.exists(self.NewMoviesDir):
            try:
                os.makedirs(os.path.expanduser(self.NewMoviesDir))
            except OSError, exc:
        ...
        if not os.path.exists(self.NonVideoDir):
            try:
                os.makedirs(os.path.expanduser(self.NonVideoDir))
            except OSError, exc:
        ...
        if not os.path.exists(self.TvdbIdFile):
...
#        _user_dict['Identifier'] = _user['Identifier']
    ...
    def BuildConfig(self, update=False):
        ...
        _base_dir = get_dir('srv', "Base Directory for Libraries")
        ...
        config['Common']['MovieGlob2'] = ["1080p", "720p", "bluray", "x264", "x264-refined", "h264",
                                         "ac3", "ac-3", "uncut", "director", "directors", "director's",
                                         "unrated", "extended", "retail", "repack", "proper", "480p",
                                         "dvdr", "dvdrip", "brrip", "bdrip", "hdtv", "ntsc", "pal", "r5",
                                         "br-screener", "screener", "tsxvid", "xvid", "divxnl", "divx",
