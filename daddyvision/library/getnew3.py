...
#        mode = self.treeselection.get_mode()
    ...
    def unlock_user_controls(self):
        ...
        if host == 'tigger':
            ...
            self.rb_michelle.set_sensitive(False)
        elif host == 'sleepy' or host == 'eeyore':
            self.user = 'michelle'
    ...
    def load_window(self):
        ...
        for entry in self.table_entries:
            self.insert_row(self.treeview_model,
                            ...
                            entry['Title'],
                            str(date.fromtimestamp(entry['Date'])),
                            entry['Path']
                            )

        if self.content == 'Series':
            self.treeview_model.set_sort_column_id(4,Gtk.SortType.ASCENDING)
        else:
            self.treeview_model.set_sort_column_id(5,Gtk.SortType.DESCENDING)
